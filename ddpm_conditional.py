...
class Diffusion:
    ...
    """
    Added this function to diffuse from a given base image
     - x: the input images to diffuse from
     - labels: the classes to diffuse into
     - t_ratio: the % of time-steps to add noise from initially (e.g. 0.5 w/ 100 time steps --> noise up to t=50)
    
    """
    def diffuse_from_base(self, x, labels, t_ratio=0.5, cfg_scale=3, use_ema=False):
        n = len(labels)
        logging.info(f"Sampling {n} new images....")
        model = self.ema_model if use_ema else self.model
        model.eval()

        # Initialize our noise
        t_start = int(self.noise_steps*t_ratio)
        x = self.noise_images(x, t_start).to(self.device)

        with torch.inference_mode():
            for i in progress_bar(reversed(range(1, t_start)), total=t_start - 1, leave=False):
    ...
    def save_model(self, run_name, use_wandb=False, epoch=-1):
        ...
        torch.save(self.model.state_dict(), os.path.join("TESTING/Diffusion-Models-pytorch-main/models", run_name, f"ckpt.pt"))
        torch.save(self.ema_model.state_dict(), os.path.join("TESTING/Diffusion-Models-pytorch-main/models", run_name, f"ema_ckpt.pt"))
        torch.save(self.optimizer.state_dict(), os.path.join("TESTING/Diffusion-Models-pytorch-main/models", run_name, f"optim.pt"))
        if use_wandb:
            at = wandb.Artifact("model", type="model", description="Model weights for DDPM conditional", metadata={"epoch": epoch})
            at.add_dir(os.path.join("TESTING/Diffusion-Models-pytorch-main/models", run_name))
            wandb.log_artifact(at)

    def prepare(self, args, mode=0):
        mk_folders(args.run_name)
        device = args.device
        self.train_dataloader, self.val_dataloader = get_data(args, mode=mode)
...
if __name__ == '__main__':
    ...
    with wandb.init(project="train_sd", group="train", config=config) if config.use_wandb else nullcontext():
        diffuser.prepare(config, mode=0)
        diffuser.fit(config)
